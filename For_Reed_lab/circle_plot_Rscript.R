...
setwd("/mnt/griffin/chrwhe/Vanessa_cardui/genome/synteny")
# Prefix
...
Qry_pal <-read_table2("Vcar_contigs.txt", col_names = F) %>%
  rename(Chr = X1) %>%
  mutate(color = "#FFFFFF")
...
#Alignment <- read_csv("nucmer_aln_Mcin_v_Vcar.qr1_filter.reformat.coords", col_names = nucmer_coord_colnames2 )
Alignment <- read_csv("nucmer_aln_Mcin_v_Vcar.qr1_u80_filter.reformat.coords", col_names = nucmer_coord_colnames2 )
...
Alignment3 <- Alignment2 %>% filter(lenQ >= 1000000 & lenR >= 1000000 & L1 >= t_len & Percent <= t_per)
